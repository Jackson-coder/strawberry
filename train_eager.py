    ...
    def train_step(imgs, yolo_loss, targets, net, optimizer, regularization):
        with tf.GradientTape() as tape:
            ...
            loss_value = yolo_loss(
                args, anchors, num_classes, label_smoothing=label_smoothing)
            if regularization:
                # 加入正则化损失
...
def fit_one_epoch(net, yolo_loss, optimizer, epoch, epoch_size, epoch_size_val, gen, genval, Epoch, anchors,
    ...
    with tqdm(total=epoch_size, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            loss_value = train_step(
                images, yolo_loss, targets, net, optimizer, regularization)
    ...
    with tqdm(total=epoch_size_val, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as pbar:
        for iteration, batch in enumerate(genval):
            ...
            pbar.set_postfix(
                **{'total_loss': float(val_loss) / (iteration + 1)})
            pbar.update(1)
    ...
    if True:
        ...
        if Use_Data_Loader:
            ...
            gen_val = tf.data.Dataset.from_generator(
                gen_val, (tf.float32, tf.float32, tf.float32))
            ...
            gen = data_generator(
                lines[:num_train], batch_size, input_shape, anchors, num_classes, mosaic=mosaic)
